...
## El algoritmo Haversine

Calcula la distancia real entre dos puntos sobre la superficie de la Tierra teniendo en cuenta su curvatura. Es la elección estándar para distancias geográficas cortas y medias.
...
|---|---|
| `dLat` / `dLon` | Diferencia de coordenadas en radianes |
| `a` | Cuadrado del semiseno del arco entre los puntos |
| `c` | Ángulo central entre los dos puntos sobre la esfera |
...
| Problema | Solución |
|---|---|
| `MainMenuController` era código muerto | Migración al patrón `@FXML` estándar de JavaFX |
| `lookup()` frágil y propenso a `null` | Inyección de componentes con `@FXML` |
| Rutas de recursos hardcodeadas rompían fuera del IDE | Carpeta `resources` marcada como Resources Root en IntelliJ |
| JSON de hospitales no se encontraba | Corrección de ruta `src/main/java/resources` → `src/main/resources` |
| `TextArea` del log siempre vacío | Uso de `FXMLLoader.getController()` en lugar de `lookup()` post-carga |
| Modo de ubicación mal detectado | Variable de estado `modoUbicacionActual` actualizada en los listeners del `SplitMenuButton` |
| Import `java.awt.event.ActionEvent` en lugar de JavaFX | Corregido a `javafx.event.ActionEvent` |
| Coordenadas invertidas en el JSON de hospitales | Documentado en el código; el filtro `tieneCoordenadasValidas()` descarta valores fuera de rango |
